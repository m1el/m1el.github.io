...
There is a good paper on theoretical impossibility of
...
## What do authors of WBC say about WBC security?
...
can be defeated with similar formulation of a problem [Wyseur09];
...
- iTunes is selling DRM-free tracks.  People like it and AAPL gets profits.
...
> And if it obfuscates even more, it changes it^(the algorithm), then it breaks with the principle of being verifiable.
...
- The only use for white-box crypto is DRM, which is a *broken task on it's own*.
