...
Then I had a discussion with my manager, on why it's difficult to make "Update" work properly.  Convincing him that I'm genuinely blocked.  

The password decryption meetings got postponed, rescheduled and kicked around.  This was dragging on for a month.  I got bored.  Since the problem involved something called "encryption", I wanted to learn cryptography.  So I went on Coursera and enrolled in "Cryptography I" by Dan Boneh.
...
- The first 8 bytes in the ciphertext are the same.  They're using a constant key and IV?
...
Three days later my lovely QAs drop one of the wildest bug reports on me.
...
Must be a server issue?  It's about 4 lines of code, there's literally nowhere to make a mistake.

I spent two days trying to narrow down the source of the issue, and there was nowhere for the bug to hide.
Can't use a debugger, because the issue is *rare enough* not to happen under a debugger.
On the third day, I get so pissed I add up to three asserts per line of code.  Asserts for every assumption I could possibly come up with.  And eventually I get a hit...

```python
decrypted = decrypt_password(encrypted)
assert encrypt_password(decrypted) == encrypted, \
  "password decryption must round trip"
