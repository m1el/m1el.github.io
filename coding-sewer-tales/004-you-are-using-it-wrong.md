...
Two Fridays layer, on a weekly checkup with my manager:
...
"Hey Igor.  There's this bug assigned for you, and it's been open for almost three months.  It's not a good look for your performance.  We need it fixed."  
...
Here's an email from two months ago with an overview.  Here's a Skype message from a month ago with a link to the JIRA ticket, comment exerpts, and a summary of the situation."  
...
Isn't the point of a GUID is to Globally IDentify an object, Uniquely?"  
...
You could claim it was a skill issue.  But this kind of roadblock has never happened to me before or after.  And yes, N was right, the access to the source code was not very productive.
...
So I run `strings` on the file, lo and behold, there are multiple strings looking like `/FOO` and `/BAR`, all bunched together.
